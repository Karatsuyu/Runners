...
if not exists:
    ...
    c.execute(
        "INSERT INTO services_providers (user_id, category_id, description, photo, resume, terms_accepted, status, approval_status, rejection_reason, approved_by_id, approved_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (1, None, 'Solicitud de prueba recreada', '', '', 0, 'INACTIVO', 'PENDIENTE', None, None, None, now, now)
    )
    conn.commit()
