...
class EmailTokenObtainPairSerializer(TokenObtainPairSerializer):
    """Accept `email` in input payload as an alias for the username field.

    This keeps the frontend sending {'email','password'} working while the
    authentication machinery expects the USERNAME_FIELD value.
    """
    def validate(self, attrs):
        # If client sent 'email' but not the expected username key, map it.
        if 'email' in attrs and self.username_field not in attrs:
