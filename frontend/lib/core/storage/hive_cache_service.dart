...
class HiveCacheService {
  ...
  static const String _contactTypesKey = 'contact_types';
  ...
  Future<void> saveContactTypes(List<Map<String, String>> types) async {
    final box = Hive.box<String>(_settingsBox);
    await box.put(_contactTypesKey, jsonEncode(types));
  }

  List<Map<String, String>>? getContactTypes() {
    final box = Hive.box<String>(_settingsBox);
    final json = box.get(_contactTypesKey);
    if (json == null) return null;
    try {
      final decoded = jsonDecode(json) as List;
      return decoded.cast<Map<String, dynamic>>().map((m) => m.map((k, v) => MapEntry(k as String, v as String))).toList();
    } catch (_) {
      return null;
    }
  }
