...
class CommerceModel {
  ...
  factory CommerceModel.fromJson(Map<String, dynamic> j) => CommerceModel(
    ...
    image: j['image'] as String?,
    // If backend provides `menu_files`, prefer the first file's URL as `menuPdf` for backward compatibility
    menuFiles: (j['menu_files'] as List?)?.map((e) => e as Map<String, dynamic>).toList(),
    menuPdf: ((j['menu_files'] as List?)?.isNotEmpty == true
            ? ((j['menu_files'] as List).first['url'] as String?)
            : null) ??
        j['menu_pdf'] as String?,
    isActive: j['is_active'] as bool? ?? true,
  );
}
...
class StoreDataSource {
  ...
  Future<List<CommerceModel>> getCommerces({int? categoryId}) async {
    try {
      ...
      final list =
          (r.data as Map<String, dynamic>)['results'] as List? ?? r.data as List;
      // Parse basic commerce models from list response
      final commerces = list
          .map((e) => CommerceModel.fromJson(e as Map<String, dynamic>))
          .toList();

      // If the list items don't include menu_files/menuPdf, enrich them by
      // fetching the commerce detail for each commerce that lacks menu info.
      // If menuFiles is empty on the list response, fetch full commerce detail
      // because the detail endpoint may include the `menu_files` array even when
      // the list endpoint provides only `menu_pdf` (legacy single-file field).
      final needDetail = commerces.where((c) => (c.menuFiles == null || c.menuFiles!.isEmpty)).toList();
      if (needDetail.isEmpty) return commerces;

      // Fetch details in parallel but limit concurrency to avoid spamming backend
      final futures = needDetail.map((c) async {
        try {
          final detail = await getCommerceDetail(c.id);
          return detail;
        } catch (_) {
          return c;
        }
      ...
      // Build map of enriched by id for quick replacement
      final enrichedMap = {for (var e in enriched) e.id: e};

      // Return commerces preserving original order, replacing with enriched when available
      return commerces.map((c) => enrichedMap[c.id] ?? c).toList();
    } on DioException catch (e) {
      throw _handleError(e);
    }
  }
  ...
  Future<CommerceModel> getCommerceDetail(int id) async {
    try {
      final r = await _dio.get('/store/commerces/$id/');
      return CommerceModel.fromJson(r.data as Map<String, dynamic>);
    } on DioException catch (e) {
      throw _handleError(e);
    }
  }
...
final commerceDetailProvider = FutureProvider.family<CommerceModel, int>((ref, id) async {
  final dataSource = ref.read(storeDataSourceProvider);
  return dataSource.getCommerceDetail(id);
});
