...
class ServicesDataSource {
  ...
  Future<List<ProviderModel>> getAdminProviders() async {
    final res = await _dio.get(
      ApiConstants.providers,
      queryParameters: {'approval_status': 'PENDIENTE'},
    );
    final data = res.data is List ? res.data as List : (res.data['results'] as List? ?? []);
    return data.map((j) => ProviderModel.fromJson(j as Map<String, dynamic>)).toList();
  }
  ...
  Future<void> registerAsProvider({
    ...
    if (cvPath != null) {
      map['resume'] = await MultipartFile.fromFile(cvPath, filename: cvPath.split('/').last);
    }
...
final providersProvider = FutureProvider<List<ProviderModel>>((ref) async {
  final categoryId = ref.watch(selectedServiceCategoryProvider);
  final authState = ref.watch(authProvider);
  // If admin, return pending providers for management
  if (authState.user?.role == UserRole.ADMIN) {
    return ref.watch(servicesDataSourceProvider).getAdminProviders();
  }
  return ref.watch(servicesDataSourceProvider).getProviders(categoryId: categoryId);
});

final adminProvidersProvider = FutureProvider<List<ProviderModel>>((ref) {
  return ref.watch(servicesDataSourceProvider).getAdminProviders();
});
