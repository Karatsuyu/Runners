...
import '../../../contacts/presentation/providers/contact_types_provider.dart';
...
void _openContactTypeManager(BuildContext context, WidgetRef ref) {
  showDialog(
    context: context,
    builder: (_) => const _ContactTypeManagerDialog(),
  );
}
...
class _ContactCategoryManagerDialog extends ConsumerWidget {
                      ...
                      trailing: Row(
                        ...
                        children: [
                          IconButton(
                            ...
                            tooltip: 'Editar',
                            onPressed: () => _openEditDialog(context, ref, c.id, c.name),
                          ),
                          IconButton(
  ...
  void _openCreateDialog(BuildContext context, WidgetRef ref) {
    showDialog(
      context: context,
      builder: (dctx) => _ContactCategoryCreateDialog(onSave: (name) async {
        try {
  ...
  void _openEditDialog(BuildContext context, WidgetRef ref, int id, String currentName) {
    showDialog(
      context: context,
      builder: (dctx) => _ContactCategoryCreateDialog(
        initialName: currentName,
        onSave: (name) async {
          try {
            final dio = ref.read(dioClientProvider).dio;
            final url = '${dio.options.baseUrl}${ApiConstants.serviceCategories}$id/';
            // debug print to help diagnose 404
            // ignore: avoid_print
            print('Admin edit category PATCH -> $url');
            await dio.patch('${ApiConstants.serviceCategories}$id/', data: {'name': name, 'description': ''});
            if (dctx.mounted) Navigator.pop(dctx);
            ref.invalidate(serviceCategoriesProvider);
          } catch (e) {
            final de = e as dynamic;
            final status = de?.response?.statusCode;
            final body = de?.response?.data ?? de?.message ?? de.toString();
            if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error ${status ?? ''}: ${body}')));
          }
        },
      ),
    );
        ...
        actions: [
          ...
          FilledButton(
              ...
              try {
                ...
                final url = '${dio.options.baseUrl}${ApiConstants.serviceCategories}$id/';
                // ignore: avoid_print
                print('Admin delete category DELETE -> $url');
                await dio.delete('${ApiConstants.serviceCategories}$id/');
                if (dctx.mounted) Navigator.pop(dctx);
                ref.invalidate(serviceCategoriesProvider);
              } catch (e) {
                final de = e as dynamic;
...
class _ContactTypeManagerDialog extends ConsumerWidget {
  const _ContactTypeManagerDialog({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final types = ref.watch(contactTypesProvider);
    return AlertDialog(
      title: const Text('Gestionar Tipos de Contacto'),
      content: SizedBox(
        width: double.maxFinite,
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ElevatedButton.icon(
              onPressed: () => _openCreateTypeDialog(context, ref),
              icon: const Icon(Icons.add),
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        IconButton(icon: const Icon(Icons.edit, size: 20), onPressed: () => _openEditTypeDialog(context, ref, t.value, t.label)),
                        IconButton(icon: const Icon(Icons.delete_outline, size: 20, color: Colors.redAccent), onPressed: () => _confirmDeleteType(context, ref, t.value, t.label)),
                      ],
                    ),
                  );
                },
  ...
  void _openCreateTypeDialog(BuildContext context, WidgetRef ref) {
    showDialog(context: context, builder: (dctx) => _ContactTypeCreateDialog(onSave: (name) async {
      try {
        await ref.read(contactTypesProvider.notifier).add(name);
        if (dctx.mounted) Navigator.pop(dctx);
        ref.invalidate(contactTypesProvider);
      } catch (e) {
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
      }
    }));
  ...
  void _openEditTypeDialog(BuildContext context, WidgetRef ref, String value, String currentLabel) {
    showDialog(context: context, builder: (dctx) => _ContactTypeCreateDialog(initialName: currentLabel, onSave: (name) async {
      try {
        await ref.read(contactTypesProvider.notifier).update(value, name);
        if (dctx.mounted) Navigator.pop(dctx);
        ref.invalidate(contactTypesProvider);
      } catch (e) {
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
  ...
  void _confirmDeleteType(BuildContext context, WidgetRef ref, String value, String label) {
    showDialog(context: context, builder: (dctx) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () async {
          try {
            await ref.read(contactTypesProvider.notifier).remove(value);
            if (dctx.mounted) Navigator.pop(dctx);
            ref.invalidate(contactTypesProvider);
          } catch (e) {
            if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e')));
...
class _ContactTypeCreateDialog extends StatefulWidget {
  final Future<void> Function(String name) onSave;
  final String? initialName;
  const _ContactTypeCreateDialog({required this.onSave, this.initialName});

  @override
  State<_ContactTypeCreateDialog> createState() => _ContactTypeCreateDialogState();
}

class _ContactTypeCreateDialogState extends State<_ContactTypeCreateDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(widget.initialName == null ? 'Crear tipo' : 'Editar tipo'),
      ...
      actions: [
        ...
        FilledButton(onPressed: _loading ? null : _save, child: _loading ? const SizedBox(width:18,height:18,child:CircularProgressIndicator(strokeWidth:2)) : const Text('Guardar')),
      ],
    );
  }
  ...
  Future<void> _save() async {
    ...
    try { await widget.onSave(name); } finally { if (mounted) setState(() => _loading = false); }
  }
}
...
class _ContactCategoryCreateDialog extends StatefulWidget {
  final Future<void> Function(String name) onSave;
  final String? initialName;
  const _ContactCategoryCreateDialog({required this.onSave, this.initialName});
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: () => ref.invalidate(adminProvidersProvider),
          ),
        ],
      ),
      body: providersAsync.when(
        ...
        data: (providers) {
          ...
          return RefreshIndicator(
            color: AppColors.primaryGreen,
            onRefresh: () async => ref.invalidate(adminProvidersProvider),
            child: ListView.separated(
              padding: const EdgeInsets.all(12),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => _openContactTypeManager(context, ref),
          ),
          IconButton(
            icon: const Icon(Icons.refresh),
      ...
      body: Builder(builder: (ctx) {
        // Keep previous data visible while refreshing to avoid the
        // full-screen loader that makes items disappear briefly.
        final prev = contactsAsync.asData?.value;
        if (contactsAsync.isLoading && prev != null) {
          final contacts = prev;
          return RefreshIndicator(
            color: AppColors.primaryGreen,
                  ...
                  AppButton(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final prevAsync = ref.read(contactsProvider);
                        ...
                        if (existing == null) {
                          ...
                          ref.read(contactsOverrideProvider.notifier).state = [tempContact, ...prev];
                          try {
                            final created = await ds.createContact(contact);
                            final replaced = ref.read(contactsOverrideProvider)!
                                .map((c) => c.id == tempId ? created : c)
                                .toList();
                            ref.read(contactsOverrideProvider.notifier).state = replaced;
                            ref.read(contactsOverrideProvider.notifier).state = null;
                            ref.invalidate(contactsProvider);
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Contacto creado correctamente'), backgroundColor: AppColors.success),
                              );
                            }
                          ...
                          // optimistic update
                          final idx = prev.indexWhere((c) => c.id == existing.id);
                          final orig = idx != -1 ? prev[idx] : existing;
                          ...
                          final newList = [...prev];
                          if (idx != -1) newList[idx] = updatedLocal;
                          ref.read(contactsOverrideProvider.notifier).state = newList;
                          try {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Contacto actualizado'), backgroundColor: AppColors.success),
                              );
                            }
                            if (ctx.mounted) Navigator.pop(ctx);
                          } catch (e) {
